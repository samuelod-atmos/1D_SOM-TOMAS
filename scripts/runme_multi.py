...
for day  in days:
  ...
  OH_proxy = 2           # 1, 2, 3
  NAME = '%s_E%s_OH%s'%(day,emiss_scheme,OH_proxy) # Name for output files
  use_fims = False       # Use the vertical profile from FIMS data? True/False

  # nucleation parameters
  # ====================================================================================================
  fion = 8.0       # Ion recombination coefficient [cm-3 s-]
  cstar_nuc = 0.0  # Volatility bound for organics to contribute to nucleation [ug m-3]
  ...
  Dbk     = [1.0E-20]  # particle-phase diffusion coefficient [m2/s] (could try 3.4e-15 from Charles) 
  ...
  Kz =  75           # Vertical diffusion coefficient 
  ...
  if day == '04272016':
    ...
    OH_lower = dt.datetime(2016,4,27,9,20)   # This is essentially the start time 
    OH_sheetname1 = 'sgpbeflux1longC1.c1.20160427.000000.custom'  
    OH_sheetname2 = 'sgpbeflux1longC1.c1.20160428.000000.custom'
    SMPS_sheetname = 'SMPS_merged_20160427'
    SMPS_lower = dt.datetime(2016,4,27,9,20)  # time at which to get the background size dist. 
    PBL_time = dt.datetime(2016,4,27,14,0)     # average time of HI-SCALE flight for PBL height
    OH_scale = 0.6313560767471572*1.4280560674520062
  elif day == '04282016':
    endtime = [10.0]
    upper_pres = 70000.0
    LR_K = 9.8 
    ...
    orgfrac_bg = 0.8 
    nh3_ppt = [5000.] 
    OH_lower = dt.datetime(2016,4,28,8,0)
    OH_sheetname1 = 'sgpbeflux1longC1.c1.20160428.000000.custom'
    OH_sheetname2 = 'sgpbeflux1longC1.c1.20160429.000000.custom'
    SMPS_sheetname = 'SMPS_merged_20160428'
    SMPS_lower = dt.datetime(2016,4,28,8,0)
    PBL_time = dt.datetime(2016,4,28,14,30)
    OH_scale = 0.7834067436507448*0.6239075885053029
  elif day == '05112016':
    endtime = [10.0]
    upper_pres = 70000.0
    LR_K = 9.8
    ...
    orgfrac_bg = 0.7
    nh3_ppt = [5000.]
    OH_lower = dt.datetime(2016, 5, 11,9,45)
    OH_sheetname1 = 'sgpbeflux1longC1.c1.20160511.000000.custom'
    OH_sheetname2 = 'sgpbeflux1longC1.c1.20160512.000000.custom'
    SMPS_sheetname = 'SMPS_merged_20160511'
    SMPS_lower = dt.datetime(2016, 5, 11,9,45)
    PBL_time = dt.datetime(2016,5,11,12,30)
    OH_scale = 0.930960974150355*1.052707603248451
  elif day == '05142016':
    endtime = [10.0]
    upper_pres = 70000.0
    LR_K = 9.8
    ...
    orgfrac_bg = 0.7
    nh3_ppt = [2000.]
    OH_lower = dt.datetime(2016, 5, 14,9,45)
    OH_sheetname1 = 'sgpbeflux1longC1.c1.20160514.000000.custom'
    OH_sheetname2 = 'sgpbeflux1longC1.c1.20160515.000000.custom' 
    SMPS_sheetname = 'SMPS_merged_20160514'
    SMPS_lower = dt.datetime(2016, 5, 14,9,45)
    PBL_time = dt.datetime(2016,5,14,12,30)
    OH_scale = 2.1898099153366086*0.5988743404938888
  elif day == '05162016':
    endtime = [10.0]
    upper_pres = 70000.0
    LR_K = 9.8 
  ...
  #PBL, RL_top, ML_top = pbl2(OH_lower,endtime,PBL_time,aadt)
  ...
  if use_fims==True:
    if day=='04272016':
      ...
      No_bg1,Dpm_bg1,sigma_bg1,No_bg2,Dpm_bg2,sigma_bg2,No_bg3,Dpm_bg3,sigma_bg3  = SD_smps_fims(SMPS_sheetname,SMPS_lower,nlayers,endtime,gc,Z_lay,temp_lay,stppres,stptemp,day,pres_lay,PBL[0])
...
#  df_emiss_ML = gsh_emission_ML.worksheet_by_title(title='%s'%emiss_sheetname).get_as_df()
  ...
  if day =='05162016' or day == '05192016':
